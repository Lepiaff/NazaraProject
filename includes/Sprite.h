...
namespace NzP
{
	...
	public:
		...
		inline virtual void UpdateGraphicsComponent(Ndk::GraphicsComponent& graphicsComponent)
		{
			m_gSet = GSetManager::Get(m_textureName);
			graphicsComponent.Attach(m_gSet->GetSprite(m_idSprite));
		}

		inline virtual void Save(const Nz::InstancedRenderableRef& renderable)
		{
